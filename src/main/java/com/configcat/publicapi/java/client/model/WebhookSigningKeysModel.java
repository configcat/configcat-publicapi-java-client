...
public class WebhookSigningKeysModel {
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!WebhookSigningKeysModel.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in WebhookSigningKeysModel is not found"
                                        + " in the empty JSON string",
                                WebhookSigningKeysModel.openapiRequiredFields.toString()));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!WebhookSigningKeysModel.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `WebhookSigningKeysModel` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
        if ((jsonObj.get("key1") != null && !jsonObj.get("key1").isJsonNull())
                && !jsonObj.get("key1").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `key1` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("key1").toString()));
        }
        if ((jsonObj.get("key2") != null && !jsonObj.get("key2").isJsonNull())
                && !jsonObj.get("key2").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `key2` to be a primitive type in the JSON string"
                                    + " but got `%s`",
                            jsonObj.get("key2").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!WebhookSigningKeysModel.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'WebhookSigningKeysModel' and its
                // subtypes
            }
            ...
            return (TypeAdapter<T>)
                    new TypeAdapter<WebhookSigningKeysModel>() {
                        @Override
                        public void write(JsonWriter out, WebhookSigningKeysModel value)
                                throws IOException {
