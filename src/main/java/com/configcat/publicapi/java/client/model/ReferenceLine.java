...
public class ReferenceLine {
    ...
    @Override
    public int hashCode() {
        return Objects.hash(lineText, lineNumber);
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!ReferenceLine.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in ReferenceLine is not found in the"
                                        + " empty JSON string",
                                ReferenceLine.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                        + " `ReferenceLine` properties. JSON: %s",
        ...
        if ((jsonObj.get("lineText") != null && !jsonObj.get("lineText").isJsonNull())
                && !jsonObj.get("lineText").isJsonPrimitive()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Expected the field `lineText` to be a primitive type in the JSON"
                                    + " string but got `%s`",
                            jsonObj.get("lineText").toString()));
        }
    }
